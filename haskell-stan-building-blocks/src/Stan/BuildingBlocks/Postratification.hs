...
psByGroupFunction :: Bool -> SB.StanBuilderM md gq (SL.Function SL.ECVec [SL.EInt, SL.EInt, SL.EIndexArray, SL.ECVec, SL.ECVec])
psByGroupFunction wgtsAreData = do
  ...
  SB.addFunctionOnce ps_by_group (SL.DataArg "Nps" :> SL.DataArg "Ngrp" :> SL.DataArg "grpPSIndex" :> wgtsArg :> SL.Arg "probs" :> TNil)
    $ \(nPS :> nGrp :> grpPSIndex :> wgts :> probs :> TNil) -> SL.writerL $ do
    let vds = SL.vectorSpec nGrp []
        plusEq = SL.opAssign SL.SAdd
--        elDivEq = SL.opAssign (SL.SElementWise SL.SDivide)
    sumByGroup <- SL.declareRHSW "SumByGroup" vds $ zeroVectorE nGrp
    sumWgts <- SL.declareRHSW "SumWgts" vds $ zeroVectorE nGrp
    SL.addStmt $ SL.for "k" (SL.SpecificNumbered (SL.intE 1) nPS)
      $ \k ->
          let atk = SL.sliceE SL.s0 k
              indexByPS = SL.indexE SL.s0 grpPSIndex
          in [atk (indexByPS sumByGroup) `plusEq` (atk wgts `SL.timesE` atk probs)
    ...
    SL.addStmt $ SL.for "l" (SL.SpecificNumbered (SL.intE 1) nGrp)
      $ \l ->
          let atl = SL.sliceE SL.s0 l
              sbgl = atl sumByGroup
              swl = atl sumWgts
              z = SL.realE 0
              gt = SL.boolOpE SL.SGT
          in [sbgl `SL.assign` SL.condE (swl `gt` z) (sbgl `SL.divideE` swl) z]
...
postStratifiedParameterF :: Bool
                         ...
                         -> SB.StanBuilderM md gq (SL.UExpr SL.ECVec)
...
--      psVecDS =  SL.vectorSpec dsSizeE [] --SB.StanVector $ SB.NamedDim dsName
  ...
  SB.inBlock block $ case reIndexRttM of
    Nothing -> do
      ...
      SB.addStmtToCode $ scopeF $ SL.writerL' $ do
        pV <- pCW
    ...
    Just (reIndexRtt, reIndex') -> do
      riProb <-  SB.stanDeclare varName $ SL.vectorSpec (SL.namedE (SB.dataSetSizeName reIndexRtt) SL.SInt) [] --(SB.StanVector $ SB.NamedDim reIndexKey) ""
      SB.addStmtToCode $ scopeF $ SL.writerL' $ do
        pV <- pCW
        wgtsV <- SL.asCW wgtsMCW
