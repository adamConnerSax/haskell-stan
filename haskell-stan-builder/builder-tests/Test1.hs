...
data ModelDataPkg = ModelDataPkg { modelRows :: SB.RowTypeTag Row, letterGroup :: SB.GroupTypeTag LetterCode }

modelDataBuilder :: SB.StanDataBuilderEff SB.ModelDataT ModelData ModelDataPkg
modelDataBuilder = do
  let modelIDT :: SB.InputDataType SB.ModelDataT ModelData = SB.ModelData
  modelDataT <- SB.addData "D1" modelIDT (SB.ToFoldable rows)
  letterGroupT <- fst <$> SB.addEnumGroup @LetterCode modelIDT "LC"
  SB.addGroupIndexForData modelIDT letterGroupT modelDataT (SB.makeIndexByCounting show letterCode)
  SB.addGroupIntMapForData modelIDT letterGroupT modelDataT (SB.dataToIntMapFromEnum letterCode)
